...
from cmdb.models.accessory import NetworkCable, Accessory, UseRecord, InventoryRecord

#
# class NetworkCableView(PermissionRequiredMixin, TemplateView):
#     permission_required = 'auth.perm_cmdb_accessory_view'
#     template_name = "cmdb/network_cable.html"
#
#     def get_context_data(self, **kwargs):
#         nc_list = NetworkCable.objects.get_queryset()
#         context = {
#             'path1': '配件',
#             'path2': '网线',
#             'idc_list': [{"id": i.id, "name": i.idc_name} for i in IDC.objects.get_queryset()],
#             'cat_list': dict_list_duplicate_delete([
#                 {'key': obj.cat, 'value': obj.get_cat_display()} for obj in nc_list]),
#             'length_list': [obj['length'] for obj in nc_list.values('length').distinct()],
#             'version_list': [{
#                 'id': obj.id,
...
class NetworkCableListView(PermissionRequiredMixin, JSONResponseMixin, TemplateView):
    permission_required = 'auth.perm_cmdb_accessory_view'
    ...
    def get(self, request, **kwargs):
        ...
        if mode_id:
            ...
            acc_detail = {
                obj.id: {
                    'version': '%s|%d米' % (obj.get_cat_display(), obj.length),
                    'cat': obj.get_cat_display(), 'length': obj.length
                }
            }
        else:
            if re.match(r'-?cat|-?length', sort_name, re.I):
                obj_list = NetworkCable.objects.get_queryset().order_by(sort_name)
            else:
                obj_list = NetworkCable.objects.get_queryset()
            cat = request.GET.get("cat", '')
            length = request.GET.get("length", '')
            if cat:
                obj_list = obj_list.filter(cat=cat)
            if length:
        ...
        if idc_id:
            ...
            if not re.match(r'-?cat|-?length', sort_name, re.I):
                accessory_list = Accessory.objects.filter(storehouse=idc, mode='network_cable', mode_id__in=mode_id_list,
            ...
            if not re.match(r'-?cat|-?length', sort_name, re.I):
                accessory_list = Accessory.objects.filter(mode='network_cable', mode_id__in=mode_id_list,
                                                          is_active=is_active).order_by(sort_name)
            else:
                accessory_list = Accessory.objects.filter(mode='network_cable', mode_id__in=mode_id_list,
                                                          is_active=is_active)
        ...
        for acc in accessory_list[int(offset):int(offset + limit)]:
            ...
            result.append({
                ...
                'cat': acc_detail[acc.mode_id]['cat'],
...
class NetworkCableDetailView(PermissionRequiredMixin, JSONResponseMixin, TemplateView):
    permission_required = ('auth.perm_cmdb_accessory_view', 'auth.perm_cmdb_accessory_edit')

    def get(self, request, **kwargs):
        pk = kwargs.get('pk')
        try:
            obj = NetworkCable.objects.get(id=pk)
            result = {
                'accessory': 'network_cable',
                'version': '%s|%d米' % (obj.get_cat_display(), obj.length)
            }
            res = {'code': 0, 'result': result}
        except NetworkCable.DoesNotExist:
    ...
    def put(self, request, *args, **kwargs):
        ...
        try:
            ...
            acc_list = Accessory.objects.filter(storehouse__id=idc_id, mode='network_cable', mode_id=nc.id, is_active=True)
            ...
            InventoryRecord.objects.create(accessory='network_cable', operate='receive',
                                           content='%s %d米 网线 %s 根！' % (nc.get_cat_display(), nc.length, count),
                                           user=request.user)
    ...
    def delete(self, *args, **kwargs):
        pk = kwargs.get('pk')
        try:
            obj = NetworkCable.objects.filter(pk=pk).delete()
            if obj:
